...
layout: post title: Shopping author: The Newist
...
{% for post in site.posts %} <div class="mdl-card mdl-cell mdl-cell--12-col"> <div class="mdl-card**media mdl-color-text--grey-50" style="background-image: url('{{ post.header_image_path }}')"> <h3><a href="{{ post.url }}">{{ post.title }}</a></h3> </div> <div class="mdl-card**supporting-text meta mdl-color-text--grey-600"> <div class="minilogo"></div> <div> <strong>{{ post.author }}</strong> <span>{{ post.date | date: &quot;%-d %B %Y&quot; }}</span> </div> </div> </div> {% endfor %} {% endraw %}
